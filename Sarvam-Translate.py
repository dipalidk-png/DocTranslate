...
BATCH_SIZE = 8
MAX_NEW_TOKENS = 512
OUTPUT_FOLDER = "salesforce_eval_outputs_Sarvam"
...
def build_prompt(src, tgt_lang):
    # Map your LANG_MAP to Sarvam's language codes
    SARVAM_LANG_CODE = {
        "GERMAN": "deu_Latn",
        "FRENCH": "fra_Latn",
        "DUTCH": "nld_Latn",
    ...
    tgt_code = SARVAM_LANG_CODE.get(tgt_lang, tgt_lang)
    return f"Translate the following text to {tgt_code}:\n{src}"
...
for lang_pair in LANG_PAIRS:
    ...
    for model_key, model_name in BASE_MODELS.items():
        ...
        tokenizer = AutoTokenizer.from_pretrained(model_name)
        # Only set pad_token if it's missing
            ...
            with torch.no_grad():
                outputs = model.generate(
                    **inputs,
                    ...
                    do_sample=False,
                    #temperature=0.1,                   #Greedy decoding does NOT use
